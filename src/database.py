...
INCOME_CATEGORIES = ["Salary", "Passive", "Gift", "Reward", "Other Active", "Bonus"]
EXPENSE_CATEGORIES = [
    ["Music", "Entertainment"],
...
class Category(Base):
    __abstract__ = True
    category = Column(
        String(MAX_CATEGORY_LENGTH), primary_key=True, nullable=False, unique=True
    )
...
class ExpenseCategory(Category):
    ...
    expense_group = Column(
        String(MAX_CATEGORY_LENGTH), ForeignKey(ExpenseGroup.category)
    )
...
class Income(Transactions):
    ...
    category = Column(String, ForeignKey(IncomeCategory.category), nullable=False)
...
def add_and_update_records(
    ...
    new_records = new_and_updated[
        ~new_and_updated[prim_key].isin(existing_records[prim_key])
    ].dropna(subset=[prim_key])
    ...
    modified_records = new_and_updated.loc[
        new_and_updated[prim_key].isin(existing_records[prim_key])
    ].dropna(subset=[prim_key])
    with session_scope(sql_engine) as session:
        for row in modified_records.iterrows():
            result = (
                session.query(table_class.__tablename__)
                .filter_by(getattr(table_class, prim_key) == row[prim_key])
                .first()
            )
...
def populate_hardcoded(sql_engine: Engine):
    ...
    add_and_update_records(
        ...
        ExpenseCategory,
        "category",
        pd.DataFrame(columns=["category", "expense_groups"], data=EXPENSE_CATEGORIES),
    )
