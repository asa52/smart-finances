...
def update_investment_values(
    eodhd_api_token, save_loc_path, investments_file, force_read_old_data=False
):
    ...
    def get_ticker_eodhd_no_token(ticker, min_date, max_date=None):
        return apis.get_ticker_values_eodhd(eodhd_api_token, ticker, min_date, max_date)

    # Data can come from two sources - different function required for each.
    get_ticker = {
        "YF": apis.get_ticker_values_yfinance,
    ...
    for _, investment in investments.iterrows():
        ...
        if isfile(fname):
            ...
            old_data.Date = pd.to_datetime(
                old_data.Date, format=DEFAULT_DATESTR_FORMAT
            ).dt.date
            latest_date = old_data.Date.max() + timedelta(days=1)
            if latest_date < date.today() and not force_read_old_data:
                new_data = get_ticker[investment.Source](investment.Ticker, latest_date)
                new_data.to_csv(fname, mode="a", header=False, index=False)
                updated_investment_data = pd.concat(
                    [old_data, new_data], axis=0, ignore_index=True
                )
            else:
...
def buy_fund(
    ...
    between_dates_idxs = fund_df.index[
        (last_trans_date < fund_df.Date) & (fund_df.Date < transaction_date)
    ]
    incl_current_idx = fund_df.index[
        (last_trans_date < fund_df.Date) & (fund_df.Date <= transaction_date)
    ]

    fund_df.iloc[between_dates_idxs, 2] = fund_df.values[last_trans_date_idx, 2]
    fund_df.loc["Shares owned"].iloc[current_trans_date_idx] = (
        fund_df["Shares owned"].iloc[current_trans_date_idx - 1] + num_shares_bought
    )
    fund_df.loc["Value"].iloc[incl_current_idx] = (
        fund_df["Shares owned"].iloc[incl_current_idx]
        * fund_df["Unit price"].iloc[incl_current_idx]
    )

    fund_df.loc["Amount invested"].iloc[between_dates_idxs] = fund_df[
        "Amount invested"
    ].iloc[last_trans_date_idx]
    fund_df.loc["Amount invested"].iloc[current_trans_date_idx] = (
        fund_df["Amount invested"].iloc[current_trans_date_idx - 1] + buy_cost
    )

    gain = (
        fund_df["Value"].iloc[incl_current_idx]
        - fund_df["Amount invested"].iloc[incl_current_idx]
    )
    fund_df.loc["% return"].iloc[incl_current_idx] = (
        gain / fund_df["Amount invested"].iloc[incl_current_idx]
    )
...
def calculate_platform_history(inputs_table, funds_dict):
    ...
    funds_history = {
        "Cash": pd.DataFrame(columns=["Date", "Value"], data=[[day_before_start, 0.0]])
    }
    last_trans_dates = {}
    for name, details in funds_dict.items():
        fund_value_transactions = details.unit_price_history_df.copy()
        fund_value_transactions[
            ["Shares owned", "Amount invested", "Value", "% return"]
        ] = 0.0
        fund_value_transactions.rename(
            columns={"Adj Close": "Unit price"}, inplace=True
        )
    ...
    for _, row in inputs_table.iterrows():
        ...
        if row["Category"] == "Transfer in":
            ...
            funds_history["Cash"].loc[cash_df_length] = [
                trans_date,
                last_cash_value + row["Price"],
            ]
        elif (
            row["Category"] == "Transfer out"
            or row["Category"] == "Fee - service"
            or row["Category"] == "Fee - advisor"
        ):
            ...
            funds_history[fund_name], last_trans_dates[fund_name] = buy_fund(
                funds_history[fund_name],
                last_trans_dates[fund_name],
                trans_date,
                shares_transferred,
                row["Price"],
                unit_price,
